...
class TestComplainant(unittest.TestCase):
    ...
    def test_complainant_attributes(self):
        """Test the initialization of the models"""
        self.assertEqual(self.example_complainant.petition_no, str(self.pet_no))
        self.assertEqual(self.example_complainant.name, 'Jane Smith')
        self.assertEqual(self.example_complainant.nationality, 'Nigerian')
        self.assertEqual(self.example_complainant.state, 'Lagos')
        self.assertEqual(self.example_complainant.gender, 'Female')
        self.assertEqual(self.example_complainant.age, 35)
        self.assertEqual(self.example_complainant.occupation, 'Teacher')
        self.assertEqual(self.example_complainant.religion, 'Christianity')
        self.assertEqual(self.example_complainant.education, 'Tertiary')
        self.assertEqual(self.example_complainant.phone_no, '08098765432')
        self.assertEqual(self.example_complainant.petition_no, str(self.pet_no))
    ...
    def test_relationship(self):
        ...
        """
        self.example_complainant.suspects.append(self.example_suspect)
        
        self.session.add(self.example_complainant)
        self.session.commit()
        
        # Check if the relationship i correctly added
        self.assertTrue(self.example_complainant.suspects)

        # Check if the relationship is correct
        complainant1 = self.session.query(Complainant).filter(
            Complainant.petition_no==str(self.pet_no)).first()
        self.assertIn(self.example_suspect, complainant1.suspects)
