...
class TestPetition(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        DIALECT_ENGINE = 'mysql+mysqldb'
        # Create Engine
        uri = "{}://{}:{}@{}/{}".format(DIALECT_ENGINE, EFCC_MYSQL_USER, EFCC_MYSQL_PWD,
    ...
    def test_cascade_delete(self):
          """Tests if the condition of casecade delete works perfectly."""
          #self.session.add(self.example_fingerprint)
          self.example_suspect.fingerprints.append(self.example_fingerprint)
          self.session.add(self.example_suspect)
          self.session.commit()
         
          self.assertIn(self.example_fingerprint, self.example_suspect.fingerprints)
         
          query = self.session.query(FingerPrint).filter(
              FingerPrint.petition_no==str(self.pet_no))
          query.delete()
          #self.session.commit()
         
          self.assertNotIn(self.example_fingerprint, self.example_suspect.fingerprints)
