...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, args):
        ...
        all_list =  args.split(' ')
        new_instance = eval(class_name)() #Creates instance of class name

        for i in range (1, len(all_list)):
            k, v = tuple(all_list[i].split("="))
            if v.startswith('"'):
                v = v.strip('"').replace("_", " ")
            else:
                try:
                    ...
                    print(f"** Can't evaluate {v}")
                    pass
            if hasattr(new_instance, k):
                setattr(new_instance, k, v) 
        
        '''
            No need of calling storage.new(new_instance) again since the BaseModel.save
            Will auto create a new instance, so calling the storage.new again will create
            another session which will conflict with the BaseModel Session.
        '''
        new_instance.save()
